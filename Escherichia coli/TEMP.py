...
def get_alalysis(G, centrality): 
    ...
    _index = 0
    for line in centrality_nodes_file:
        if (_index >= 1): # for the header
            centrality_based_nodes.append(line.split('\t')[0])
            centrality_based_nodes_centrality[line.split('\t')[0]] = append(line.split('\t')[1])
        _index += 1
  
    centrality_dictionary = get_centrality(G, centrality)

    essential_proteins_info = get_essential_proteins_info()
    [_IDs, _Essentiality, essential_IDs, unknown_essential_IDs]
