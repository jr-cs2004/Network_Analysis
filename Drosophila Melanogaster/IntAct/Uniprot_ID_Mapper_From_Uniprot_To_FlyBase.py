...
all_unique_proteins_in_intact = []
all_unique_proteins_in_intact.extend(intact_source_protein_IDs)
all_unique_proteins_in_intact.extend(intact_target_protein_IDs)

_str = '# of all_unique_proteins_in_intact: ' + str(len(list(set(all_unique_proteins_in_intact)))) + '\n'
print(_str)
statistics_file.write(_str)
...
query = ' '.join(intact_source_protein_IDs)
intact_source_to_flybase_IDs = convertIDs(query, 'ID', 'FLYBASE_ID')

_str = ('IntAct IDs are mapped to flybase IDs:' + '\n'
+ '# of IDs returned by uniprot Mapper: ' + '\n'
+ 'Source: ' + '\n'
+ 'from: ' + str(len(intact_source_to_flybase_IDs[0])) + ', and to: ' + str(len(intact_source_to_flybase_IDs[1]))
+ ', and uniques --> from: ' + str(len(list(set(intact_source_to_flybase_IDs[0])))) +', and to: ' +  str(len(list(set(intact_source_to_flybase_IDs[1])))) + '\n')
print(_str)
statistics_file.write(_str)

query = ' '.join(intact_target_protein_IDs)
intact_target_to_flybase_IDs = convertIDs(query, 'ID', 'FLYBASE_ID')

_str = ('# of IDs returned by uniprot Mapper: ' + '\n'
+ 'Target: ' + '\n'
+ 'from: ' + str(len(intact_target_to_flybase_IDs[0])) + ', and to: ' + str(len(intact_target_to_flybase_IDs[1]))
+ ', and uniques --> from: ' + str(len(list(set(intact_target_to_flybase_IDs[0])))) + ', and to: ' + str(len(list(set(intact_target_to_flybase_IDs[1])))) + '\n')
print(_str)
statistics_file.write(_str)
...
all_intact_proteins_having_at_least_an_ID_in_flybase = []
all_intact_proteins_having_at_least_an_ID_in_flybase.extend(intact_source_to_flybase_IDs[0])
all_intact_proteins_having_at_least_an_ID_in_flybase.extend(intact_target_to_flybase_IDs[0])

all_flybase_IDs_obtained_by_mapping_uniprot_to_flybase = []
all_flybase_IDs_obtained_by_mapping_uniprot_to_flybase.extend(intact_source_to_flybase_IDs[1])
all_flybase_IDs_obtained_by_mapping_uniprot_to_flybase.extend(intact_target_to_flybase_IDs[1])

_str = ('# of all_intact_proteins_having_at_least_an_ID_in_flybase: '
+ str(len(list(set(all_intact_proteins_having_at_least_an_ID_in_flybase)))) + '\n'
...
intact_source_to_flybase_hash = {}
intact_target_to_flybase_hash = {}
all_intact_to_flybase_hash = {}

# for x in list(set(intact_source_to_flybase_IDs[0])):
for x in list(set(intact_source_protein_IDs)):
   intact_source_to_flybase_hash[x] = []
   all_intact_to_flybase_hash[x] = []

n = len(intact_source_to_flybase_IDs[0])
for i in range(0, n): 
   intact_source_to_flybase_hash[intact_source_to_flybase_IDs[0][i]].append(intact_source_to_flybase_IDs[1][i])
   all_intact_to_flybase_hash[intact_source_to_flybase_IDs[0][i]].append(intact_source_to_flybase_IDs[1][i])

# for x in list(set(intact_target_to_flybase_IDs[0])):
for x in list(set(intact_target_protein_IDs)):
   intact_target_to_flybase_hash[x] = []
   all_intact_to_flybase_hash[x] = []

n = len(intact_target_to_flybase_IDs[0])
for i in range(0, n): 
...
for key in all_intact_to_flybase_hash:
   lst_1 = list(set(all_intact_to_flybase_hash[key]))
   flag = True
   for key_2 in all_intact_to_flybase_hash:
      lst_2 = list(set(all_intact_to_flybase_hash[key_2]))
      if (key != key_2):
...
for key in all_intact_to_flybase_hash:
   if (len(list(set(all_intact_to_flybase_hash[key]))) == 0):
      counter_0 += 1
