...
def get_alalysis(G, centrality): 
    ...
    for i in range(50, 401, 50): # for i in range(50, 951, 50):
        ...
        critical_nodes_N_centrality_based_nodes = [value for value in critical_nodes if value in centrality_based_nodes[0:371]]

        _lst = [value for value in critical_nodes_N_centrality_based_nodes if value in critical_nodes_N_essentials]
        print('essential_nodes_intersection:\t' + str(len(_lst)) + '\t' + str(len(_lst) / len(critical_nodes_N_essentials) * 100) + '%')
        print('Number of nodes:\t', i)
        print('critical_nodes_N_essentials:\t', len(critical_nodes_N_essentials))
        print('critical_nodes_N_centrality_based_nodes:\t', len(critical_nodes_N_centrality_based_nodes))
        _temp = [value for value in essential_proteins_ID if value in centrality_based_nodes[0:371]]
        print(len(_temp))
        print('\n\n')        
